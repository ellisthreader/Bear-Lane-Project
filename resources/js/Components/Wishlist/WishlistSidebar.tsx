      ...
      {showWishlist && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 flex h-[100dvh] w-full max-w-full flex-col border-l border-[#C6A75E]/25 bg-[#FAFAF7] text-gray-900 shadow-2xl sm:w-[30rem]"
          >
            <motion.div
              initial={{ y: -8, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.06, duration: 0.28 }}
              className="flex items-center justify-between border-b border-[#C6A75E]/25 bg-gradient-to-r from-[#F8F3E6] via-[#FCFAF2] to-white px-4 py-4 sm:px-5"
            >
              ...
              {wishlist.length === 0 ? (
                ...
                wishlist.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-3 border-b border-gray-200 pb-4">
                    <div className="h-16 w-16 flex-shrink-0 overflow-hidden rounded-xl border border-gray-200 bg-white sm:h-20 sm:w-20">
                      {item.image ? (
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 line-clamp-2 text-sm font-semibold text-[#2A2314]">{item.name}</p>
            ...
            <div className="border-t border-[#C6A75E]/20 bg-white p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4">
              <button
                onClick={toggleWishlist}
